...
int save_png(const char* filename, const unsigned char* image_data, int width, int height) {
    FILE *fp;
    png_structp png_ptr;
    png_infop info_ptr;
    ...
    row_pointer = (unsigned char*) image_data;
...
int main( int argc, char** argv, char** envv){
	...
	while((opt = getopt(argc,argv, "w:h:o:")) != -1) {
		switch (opt) {
			...
			case 'o':
				...
				break;
			default: /* '?' */
				fprintf(stderr,"Usage: %s [-w cols] [-h rows] [-o output_directory] id_camera\n", argv[0] ) ;
				exit(EXIT_FAILURE) ;
		}
	}
	...
	if ( camera_name == NULL ) {
		fprintf(stderr, "No camera given.\n");
		exit(EXIT_FAILURE) ;
	}
